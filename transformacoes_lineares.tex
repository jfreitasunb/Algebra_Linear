...
\begin{exemplos}
	\begin{enumerate}
		\item Considere $V = \mathcal{C}^2$ como um $\mathcal{C}$-espaço vetorial e seja $\mathcal{B} = \{(1,i);(i,0)\}$ uma base de $\mathcal{C}^2$ \textit{(Verifique!)}. Dado o vetor $v = (i, 2+i) \in \mathcal{C}^2$, quais suas coordenadas em relação a tal base?
		\begin{solucao}
			As coordenadas de $v$ em relação à base $\mathcal{B}$ serão $[v]_\mathcal{B} = (\alpha_1,\alpha_2)_\mathcal{B}$ onde $\alpha_1$, $\alpha_2 \in \mathcal{C}^2$ são tais que
			\[
