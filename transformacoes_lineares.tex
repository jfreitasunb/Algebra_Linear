...
\begin{teorema}\label{matriz_da_composicao_de_transformacoes}
	Sejam $F : V \to W$ e $G : W \to U$ duas transformações lineares onde $V$, $W$ e $U$ são $\cp{K}$-espaços vetoriais de dimensões $p$, $q$ e $r$, respectivamente. Fixe bases ordenadas $\mathcal{B}_V$, $\mathcal{B}_W$ e $\mathcal{B}_U$ para $V$, $W$ e $U$, respectivamente. Então $(G \circ F) : V \to U$ dada por $(G\comp F)(v) = G(F(v))$ é uma transformação linear e
	\[
