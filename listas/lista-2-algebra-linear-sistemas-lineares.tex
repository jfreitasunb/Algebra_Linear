...
\begin{exercicio}
  ...
  \[
    ...
    \end{bmatrix}
  \]
  uma matriz $2 \times 2$ com elementos complexos. Suponhamos que $A$ seja linha-reduzida e também que $a + b + c + d = 0$. Mostre que existem exatamente três destas matrizes.
\end{exercicio}

\begin{exercicio}
  Demonstrar que as duas matrizes seguintes \textbf{não} são linha-equivalentes:
  \[
    \begin{bmatrix}
      2 & 0 & 0\\
      a & -1 & 0\\
...
\begin{exercicio}
  ...
  \[
    ...
    \end{bmatrix}.
  \]
  Para que ternas $(y_1, y_2, y_3)$ o sistema $AX = Y$ admite solução? Considere $y_i \in \real$, $i=1$, 2, 3.
\end{exercicio}
...
\begin{enumerate}[label={\alph*})]
  \item Encontre a solução geral do sistema $(A + 4I_3)X = 0$ em $\real$.
  \item Encontre a solução geral do sistema $(A - 2I_3)X = 0$ em $\real$.
\end{enumerate}
\begin{solucao}
  \begin{enumerate}[label={\alph*})]
...
\begin{exercicio}
  Encontre condições sobre os $b_i \in \real$'s para que cada um dos sistemas tenha solução.
  \begin{enumerate}[label={\alph*})]
    \item $\begin{cases}
      x_1 - 2x_2 + 5x_3 = b_1\\
      4x_1 - 5x_2 + 8x_3 = b_2\\
